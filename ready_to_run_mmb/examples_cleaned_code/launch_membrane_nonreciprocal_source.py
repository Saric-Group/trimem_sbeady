...
# - The N = 5072 membrane has Radius = 20                          #
# -----------------------------------------------------------------#
...
radius_membrane = np.mean(np.sqrt(mesh.vertices[:, 0]**2 + mesh.vertices[:, 1]**2 + mesh.vertices[:, 2]**2))
print(f"Radius membrane : {radius_membrane}")
...
# symbiont properties
mass_symbiont  = 1.0
sigma_symbiont = (2.0*radius_membrane)*0.8
...
# integrators
postequilibration_commands.append("fix NVEALL vertices nve")
postequilibration_commands.append(f"fix LGVALL vertices langevin {temperature} {temperature} {langevin_damp} {langevin_seed} zero yes")
